...
(function () {
  ...
  (function heroAura() {
    ...
    var FS = "precision highp float;\n" +
      ...
      "  gl_FragColor = vec4(col, 0.52*vig);\n" + // fond sombre : glow renforce
      "}";
  ...
  /* -------- NAV flottante au-dessus du hero sombre, solide au scroll -------- */
  (function navFloat() {
    var nav = document.querySelector(".nav"), hero = document.getElementById("top");
    if (!nav || !hero) return;
    function upd() {
      var h = hero.offsetHeight - 90;
      if ((window.scrollY || window.pageYOffset) < h) nav.classList.add("is-float");
      else nav.classList.remove("is-float");
    }
